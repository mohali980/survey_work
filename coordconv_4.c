...
void coordconv_4()
{
	...
	ifstream in_file;
	ofstream out_file,pre_out_file,out_trees_file,out_file_full;
...
//	1) END : ................	reading data from TXT files	...................
	...
	out_file_full.open("out_file_full.txt");


	for(int i=0;i<x_vec.size();i++)
	{
		out_file_full<<file_name_vec[i]<<","<<frame_vec[i]<<","<<collection_vec[i]<<","<<group_vec[i]<<","<<point_vec[i]<<","<<x_vec[i]<<","<<y_vec[i]<<","<<z_vec[i]<<" ,  "<<offp_vec[i]<<","<<offr_vec[i]<<","<<date_time_vec[i]<<endl;
	}
...
cout<<"dfsdfsdfsdfsdfsdfs"<<endl;
	...
	basic_string<char> a ;
	TString treeeeee=tmp_group_0+"_"+(TString)to_string (num_trees);
