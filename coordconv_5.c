...
//	storing all the points from the vector to a TXT file.
	...
	ofstream out_file_5_0;

	out_file_5_0.open("out_file_5_0.txt");


	for(int i=0;i<x_vec.size();i++)
	{
		out_file_5_0<<file_name_vec[i]<<","<<frame_vec[i]<<","<<collection_vec[i]<<","<<group_vec[i]<<","<<point_vec[i]<<","<<x_vec[i]<<","<<y_vec[i]<<","<<z_vec[i]<<" ,  "<<offp_vec[i]<<","<<offr_vec[i]<<","<<date_time_vec[i]<<endl;
	}
...
//	defining the output root file.
	...
	TFile *	out_root_file	=	new TFile("points_5.root","RECREATE");

//	----------------------------------------------------------------------------
//	2) START :	seperating the known points by using the common group's name. ..
//	----------------------------------------------------------------------------

	cout<<"START Block (2) : seperating the known points by using the common group's name. "<<endl;

//	inserting the known groups, using the excil file.

	vector<TString> known_groups_vec=
	{"Panel5O1","Schlitz","Panel_1","Panel_2","Panel_3","Panel_4","Panel_5","Panel_6","Panel_7" ,"Panel_8","Panel_9","Panel_10"
	,"Panel_11","Panel_12","Panel_13","Panel_14","Panel_15","Panel_1_ebeneDS1","Panel_1_ebeneDS2","Panel_1_Ecken"};

//	...................................
	ofstream out_trees_file;
	out_trees_file.open("trees_file_5.txt");
	for(int i=0;i<known_groups_vec.size();i++){out_trees_file<<known_groups_vec[i]<<endl;}
	out_trees_file.close();
//	...................................


	vector<int> same_group,same_xyz;


	ofstream out_file_5_1;
	out_file_5_1.open("out_file_5_1.txt");

	for(int i=0;i<known_groups_vec.size();i++)
	{


		for(int j=0; j<point_vec.size();j++)
		{
			if(group_vec[j]==known_groups_vec[i])
			{
				same_group.push_back(j);

			}	//	if(group_vec[j]==known_groups_vec[i])
			else	{same_group.push_back(0);}
		}		//	for(j) over all the points

		
		for(int k=0;k<same_group.size();k++)
		{


			float tmp_x=x_vec[same_group[k]];
			float tmp_y=y_vec[same_group[k]];
			float tmp_z=z_vec[same_group[k]];
			int count=0;

			for(int l=k+1;l<same_group.size();l++)
			{
				if(tmp_x==x_vec[same_group[l]] && tmp_y==y_vec[same_group[l]] && tmp_z==z_vec[same_group[l]] && count<1 )
				{
					count++;
					same_xyz.push_back(same_group[l]);
				}
			}
	...
	for(int j=0;j<same_xyz.size();j++)
	{
		cout<<group_vec[same_xyz[j]]<<"	"<<x_vec[same_xyz[j]]<<"	"<<y_vec[same_xyz[j]]<<"	"<<z_vec[same_xyz[j]]<<endl;
	}
	...
	}			//	for(i) over the known groups vector
...
//	...................................
//		out_file_5_1<<file_name_vec[j]<<","<<frame_vec[j]<<","<<collection_vec[j]<<","<<group_vec[j]<<","<<point_vec[j]<<","<<x_vec[j]<<","<<y_vec[j]<<","<<z_vec[j]<<" ,  "<<offp_vec[j]<<","<<offr_vec[j]<<","<<date_time_vec[j]<<endl;
//	...................................
...
//	storing all the points from the vector to a TXT file.
	...
	ofstream out_file_5_2;

	out_file_5_2.open("out_file_5_2.txt");


	for(int i=0;i<x_vec.size();i++)
	{
		out_file_5_2<<file_name_vec[i]<<","<<frame_vec[i]<<","<<collection_vec[i]<<","<<group_vec[i]<<","<<point_vec[i]<<","<<x_vec[i]<<","<<y_vec[i]<<","<<z_vec[i]<<" ,  "<<offp_vec[i]<<","<<offr_vec[i]<<","<<date_time_vec[i]<<endl;
	}
